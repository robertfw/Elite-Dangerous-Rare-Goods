...
class System(Base):
    ...
    def import_(cls, sheet, session):
        ...
        # add 1 because of range() behavior
    ...
    def import_(cls, sheet, session):
        ...
        for row in range(FIRST_ROW, LAST_ROW + 1):  # add 1 for range behavior
            ...
            if strip(get_cell_value(sheet, SUP_RATE + row)).upper() == 'ND':
                ...
                min_supply = round(
                    float(get_cell_value(sheet, MIN_SUPPLY + row)))
                max_supply = round(
                    float(get_cell_value(sheet, MAX_SUPPLY + row)))

            price = round(float(get_cell_value(sheet, PRICE + row)))

            system = session.query(System).filter_by(name=sys).first()
            station = session.query(Station).filter_by(
                system=system, name=stt).first()

            Goods.add(
                session, station, name, max_cap, min_supply, max_supply, price)

    @classmethod
    def add(cls, session, station, name, max_cap, min_supply, max_supply, price):
        session.add(Goods(station=station, name=name, max_cap=max_cap,
                          min_supply=min_supply, max_supply=max_supply, price=price))
...
def optimize(goods, outputs=1, max_dist=None, max_cargo=None):
    ...
    while len(goods) > 0:
        ...
        for destination in goods:
            ...
            if max_dist is None or dist <= max_dist:
                ...
                profit_out = oes * \
                    (sale_price(origin, destination.station.system) -
                     origin.price)
                profit_back = des * \
                    (sale_price(destination, origin.station.system) -
                     destination.price)
                rt_profit = profit_out + profit_back
                results.append((rt_profit, origin, destination, dist))

    # sorting by negative profit gives us the highest profits first
    results.sort(key=lambda ptuple: -ptuple[0])
    ...
    for r in range(outputs):
        ...
        print(
            " at a distance of {:.2f} Ly with round-trip expected profit {:.0f}".format(dist, profit))
        print()
...
if __name__ == '__main__':
    ...
    if not any([ns.wipe, ns.count, ns.display, ns.optimize]):
        ...
        with session_scope(Session) as session:
            ...
            if ns.optimize:
                optimize(
                    q.all(), ns.optimize_outputs, ns.max_dist, ns.limit_cargo)
